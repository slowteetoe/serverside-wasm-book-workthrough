use image::{imageops::Triangle, ImageReader};
use ndarray::Array;
use std::io::BufRead;
use std::{env, fs};

wit_bindgen::generate!({path: "wit/wasi-nn.wit", world: "ml"});
...
fn main() {
    ...
    let class_labels: Vec<_> = labels_data.lines().map(|line| line.unwrap()).collect();
    ...
    let output_data = exec_context
        .get_output("squeezenet0_flatten0_reshape0")
        .unwrap()
        .data();
